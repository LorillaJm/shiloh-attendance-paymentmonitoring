                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $this->getSessions(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $session): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <tr>
                                    <td class="px-4 py-3"><?php echo e($session->session_date->format('M d, Y')); ?></td>
                                    <td class="px-4 py-3"><?php echo e($session->sessionType->name); ?></td>
                                    <td class="px-4 py-3"><?php echo e(\Carbon\Carbon::parse($session->start_time)->format('H:i')); ?> - <?php echo e(\Carbon\Carbon::parse($session->end_time)->format('H:i')); ?></td>
                                    <td class="px-4 py-3"><?php echo e($session->teacher?->name ?? '-'); ?></td>
                                    <td class="px-4 py-3">
                                        <span class="px-2 py-1 text-xs rounded-full 
                                            <?php if($session->status === 'COMPLETED'): ?> bg-green-100 text-green-800
                                            <?php elseif($session->status === 'CANCELLED'): ?> bg-red-100 text-red-800
                                            <?php elseif($session->status === 'NO_SHOW'): ?> bg-orange-100 text-orange-800
                                    ...
                                    <td class="px-4 py-3">
                                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($session->attendanceRecord): ?>
                                            <span class="px-2 py-1 text-xs rounded-full 
                                                <?php if($session->attendanceRecord->status === 'PRESENT'): ?> bg-green-100 text-green-800
                                                <?php elseif($session->attendanceRecord->status === 'ABSENT'): ?> bg-red-100 text-red-800
                                                <?php elseif($session->attendanceRecord->status === 'LATE'): ?> bg-yellow-100 text-yellow-800
                                    ...
                                    <td class="px-4 py-3 text-sm"><?php echo e($session->notes); ?></td>
                                </tr>
