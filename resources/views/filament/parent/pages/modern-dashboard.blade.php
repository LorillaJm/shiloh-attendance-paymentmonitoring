<x-filament-panels::page>
    ...
    {{-- Include Theme CSS --}}
    @push('styles')
        <link rel="stylesheet" href="{{ asset('css/parent-theme.css') }}">
    @endpush
    ...
    @else
        ...
        @if($data['alerts']['total'] > 0)
            <div style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); border-radius: 16px; padding: 1.25rem 1.5rem; margin-bottom: 1.5rem; display: flex; align-items: center; gap: 1rem; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
                ...
                <div style="flex: 1;">
                    <p style="color: white; font-weight: 600; font-size: 0.875rem; margin-bottom: 0.25rem;">You have {{ $data['alerts']['total'] }} notification(s) requiring attention</p>
                    <p style="color: rgba(255, 255, 255, 0.9); font-size: 0.75rem;">
                        @if($data['alerts']['overdue_payments'] > 0)
                            {{ $data['alerts']['overdue_payments'] }} overdue payment(s)
                        @endif
                        @if($data['alerts']['upcoming_payments'] > 0)
                            {{ $data['alerts']['upcoming_payments'] }} upcoming payment(s)
                        @endif
                        @if($data['alerts']['low_sessions'] > 0)
                            {{ $data['alerts']['low_sessions'] }} low session alert(s)
                        @endif
                    </p>
                </div>
                <a href="{{ route('filament.parent.pages.notifications') }}" style="padding: 0.625rem 1.25rem; background: white; color: #f5576c; border-radius: 10px; font-weight: 600; font-size: 0.875rem; text-decoration: none; transition: transform 0.2s; flex-shrink: 0;">
                    View All
                </a>
            </div>
        @endif
                ...
                <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 20px; padding: 2rem; margin-bottom: 1.5rem; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1);">
                    <div style="display: flex; align-items: center; gap: 1.5rem; flex-wrap: wrap;">
                        ...
                        <div style="flex: 1; min-width: 200px;">
                            <h2 style="font-size: 1.875rem; font-weight: 700; color: white; margin-bottom: 0.5rem;">{{ $student->full_name }}</h2>
                            <p style="font-size: 0.875rem; color: rgba(255, 255, 255, 0.9); margin-bottom: 0.25rem;">{{ $student->student_no }}</p>
                            <p style="font-size: 0.875rem; color: rgba(255, 255, 255, 0.8);">{{ $student->package_name }}</p>
                        </div>
                        <div style="display: flex; align-items: center; gap: 0.75rem;">
                            <span style="padding: 0.5rem 1rem; background: {{ $student->status === 'ACTIVE' ? 'rgba(16, 185, 129, 0.2)' : 'rgba(107, 114, 128, 0.2)' }}; color: white; border-radius: 9999px; font-size: 0.75rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em; backdrop-filter: blur(10px);">
                                {{ $student->status }}
                            </span>
                        </div>
                    </div>
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 1.25rem; margin-bottom: 1.5rem;">
                    ...
                    <div style="background: white; border-radius: 16px; padding: 1.5rem; box-shadow: 0 1px 3px 0 rgba(0, 0, 0, 0.1); border: 1px solid #f3f4f6; transition: all 0.2s;">
                        ...
                        @if($student->today_attendance?->remarks)
                            <p style="font-size: 0.75rem; color: #9ca3af;">{{ Str::limit($student->today_attendance->remarks, 40) }}</p>
                        @endif
                    </div>
                    ...
                    <div style="background: white; border-radius: 16px; padding: 1.5rem; box-shadow: 0 1px 3px 0 rgba(0, 0, 0, 0.1); border: 1px solid #f3f4f6; transition: all 0.2s;">
                        ...
                        <p style="font-size: 1.875rem; font-weight: 700; color: #1f2937;">₱{{ number_format($student->remaining_balance, 2) }}</p>
                    </div>
                    ...
                    <div style="background: white; border-radius: 16px; padding: 1.5rem; box-shadow: 0 1px 3px 0 rgba(0, 0, 0, 0.1); border: 1px solid #f3f4f6; transition: all 0.2s;">
                        ...
                        <p style="font-size: 1.875rem; font-weight: 700; color: #10b981;">₱{{ number_format($student->total_paid, 2) }}</p>
                        <p style="font-size: 0.75rem; color: #9ca3af;">of ₱{{ number_format($student->total_fee, 2) }} total fee</p>
                    </div>
                    ...
                    <div style="background: white; border-radius: 16px; padding: 1.5rem; box-shadow: 0 1px 3px 0 rgba(0, 0, 0, 0.1); border: 1px solid #f3f4f6; transition: all 0.2s;">
                        ...
                        <p style="font-size: 1.875rem; font-weight: 700; color: #1f2937; margin-bottom: 0.25rem;">{{ $student->remaining_sessions }}</p>
                        <p style="font-size: 0.75rem; color: #9ca3af;">of {{ $student->total_sessions }} total</p>
                    </div>
                    ...
                    <div style="background: white; border-radius: 16px; padding: 1.5rem; box-shadow: 0 1px 3px 0 rgba(0, 0, 0, 0.1); border: 1px solid #f3f4f6; transition: all 0.2s;">
                        ...
                        <p style="font-size: 1.875rem; font-weight: 700; color: #1f2937; margin-bottom: 0.25rem;">{{ $data['summary']['attendance']['present'] }}</p>
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 1.5rem; margin-top: 2rem;">
            ...
            <div style="background: white; border-radius: 16px; padding: 1.5rem; box-shadow: 0 1px 3px 0 rgba(0, 0, 0, 0.1); border: 1px solid #f3f4f6;">
                ...
                <div style="display: flex; flex-direction: column; gap: 0.75rem;">
                    @forelse($data['recent_attendance'] as $record)
                        <div style="display: flex; align-items: center; justify-content: space-between; padding: 0.875rem; background: #f9fafb; border-radius: 12px; transition: all 0.2s;">
                            <div style="flex: 1;">
                                <p style="font-size: 0.875rem; font-weight: 600; color: #1f2937; margin-bottom: 0.25rem;">{{ $record->student->full_name }}</p>
                                <p style="font-size: 0.75rem; color: #6b7280;">{{ $record->attendance_date->format('M d, Y') }}</p>
                            </div>
                            <span style="padding: 0.375rem 0.75rem; border-radius: 9999px; font-size: 0.75rem; font-weight: 600; 
                                ...
                                {{ $record->status === 'EXCUSED' ? 'background: #dbeafe; color: #1e40af;' : '' }}">
                                {{ $record->status }}
                            </span>
                        </div>
                    @empty
                ...
                <a href="{{ route('filament.parent.pages.my-children-attendance') }}" style="display: block; margin-top: 1rem; text-align: center; color: #667eea; font-size: 0.875rem; font-weight: 600; text-decoration: none;">
                    View all attendance →
                </a>
            </div>
            ...
            <div style="background: white; border-radius: 16px; padding: 1.5rem; box-shadow: 0 1px 3px 0 rgba(0, 0, 0, 0.1); border: 1px solid #f3f4f6;">
                ...
                <a href="{{ route('filament.parent.pages.my-children-payments') }}" style="display: block; margin-top: 1rem; text-align: center; color: #667eea; font-size: 0.875rem; font-weight: 600; text-decoration: none;">
                    View all payments →
                </a>
            </div>
            ...
            <div style="background: white; border-radius: 16px; padding: 1.5rem; box-shadow: 0 1px 3px 0 rgba(0, 0, 0, 0.1); border: 1px solid #f3f4f6;">
                ...
                <div style="display: flex; flex-direction: column; gap: 0.75rem;">
                    @forelse($data['upcoming_sessions'] as $session)
                        <div style="padding: 0.875rem; background: #f9fafb; border-radius: 12px; transition: all 0.2s;">
                            <p style="font-size: 0.875rem; font-weight: 600; color: #1f2937; margin-bottom: 0.25rem;">{{ $session->sessionType->name }}</p>
                            <p style="font-size: 0.75rem; color: #6b7280; margin-bottom: 0.125rem;">{{ $session->student->full_name }}</p>
                ...
                <a href="{{ route('filament.parent.pages.my-children-sessions') }}" style="display: block; margin-top: 1rem; text-align: center; color: #667eea; font-size: 0.875rem; font-weight: 600; text-decoration: none;">
                    View all sessions →
                </a>
            </div>
        </div>
